...
def globalValLexer(prog, pos, long):
...
def getToken(imprime=True):
    ...
    while posicion < progLong:
        ...
        elif estado == 19: # Ajustado para ser ignorado por el parser
            # Estado 19: '/*', (iniciando comentario multilinea)
            while posicion < progLong and not (programa[posicion] == '*' and programa[posicion + 1] == '/'):
                if programa[posicion] == '\n':
                    lineaActual += 1  # Contamos las líneas dentro del comentario
                posicion += 1
            if posicion < progLong:
                posicion += 2  # Avanzamos dos posiciones (el '*' y el '/')
                # Ignoramos el comentario y continuamos con el siguiente token
                tokenString = ''  # Reiniciamos el tokenString
                estado = 0  # Regresamos al estado inicial
                continue  # Volvemos al inicio del bucle para procesar el siguiente token
            else:
                # Si no encontramos el cierre del comentario, reportamos un error
                token = TokenType.ERROR
                tokenString = '/*'  # El token erróneo es solo '/*'
                mensajeError(lineaActual, tokenString)
                posicion += 1  # Avanzamos una posición para continuar el análisis
