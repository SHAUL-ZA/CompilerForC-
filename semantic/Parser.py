...
def match(esperado):
    global tokenActual
    # print(f"match: esperando {esperado}, actual {tokenActual}") # Debug
...
def program():
    ...
    # print("Iniciando análisis del programa") # Debug
    nodo = Nodo("programa")
    while posicion < progLong and tokenActual[0] != TokenType.ENDFILE:
        # print(f"program: procesando token {tokenActual}") # Debug
...
# ----------------------------
def declaracion():
    # print(f"declaracion: token actual {tokenActual}") # Debug
    if tokenActual[0] in [TokenType.INT, TokenType.VOID]:
        ...
        if tokenActual[0] == TokenType.ID:
            ...
            if tokenActual[0] == TokenType.LPAREN:
                # print(f"Declaración de función: {id_name}") # Debug
            ...
            # Declaración de variable
                ...
                # print(f"Declaración de variable: {id_name}") # Debug
                match(TokenType.SEMI)
                var_node = Nodo("var-decl", id_name)
                var_node.agregarHijo(tipo)
                return var_node
            elif tokenActual[0] == TokenType.LBRACKET:  # Declaración de arreglo
                # print(f"Declaración de arreglo: {id_name}") # Debug
                ...
                if tokenActual[0] == TokenType.NUM:
                    ...
                    if tokenActual[0] == TokenType.SEMI:
                        match(TokenType.SEMI)
                        arr_node = Nodo("array-decl", id_name)
                        arr_node.agregarHijo(tipo)
                        arr_node.agregarHijo(Nodo("num", num_value))
                        return arr_node
                    else:
                        errorSintactico("Se esperaba ';' después de la declaración de arreglo")
            else:
...
def param():
    global tokenActual
    tipo = tipo_espec()  # Obtener el tipo del parámetro (int o void)
    if tokenActual[0] == TokenType.ID:
        nombre = tokenActual[1]  # Obtener el nombre del parámetro
        match(TokenType.ID)
        if tokenActual[0] == TokenType.LBRACKET:  # Verificar si es un arreglo
            match(TokenType.LBRACKET)
            if tokenActual[0] == TokenType.RBRACKET:
                match(TokenType.RBRACKET)
                return Nodo("param", f"{nombre}[]")  # Nodo para un arreglo
            else:
                errorSintactico("Se esperaba ']' después de '['")
        # Agregar el tipo como hijo del nodo param
        param_node = Nodo("param", nombre)
        param_node.agregarHijo(tipo)  # Agregar el tipo como hijo del nodo param
        return param_node
    else:
...
def factor():
    if tokenActual[0] == TokenType.LPAREN:
        ...
        if tokenActual[0] == TokenType.LBRACKET:  # Acceso a un arreglo
            match(TokenType.LBRACKET)
            expr = expression()  # Procesar la expresión dentro de los corchetes
            match(TokenType.RBRACKET)
            var_node = Nodo("array-access", id_name)  # Nodo para acceso a arreglo
            var_node.agregarHijo(expr)  # Agregar la expresión como hijo
            return var_node
        else:
